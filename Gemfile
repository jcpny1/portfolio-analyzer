...
gem 'rails', '~> 6.0.3.4'      # Full-stack web framework.
gem 'pg', '~> 1.2', '>= 1.2.3' # Use postgresql as the database for Active Record.
gem 'puma', '~> 5.0.4'         # Use Puma as the (HTTP 1.1) app server.
...
gem 'jbuilder', '~> 2.10.1'    # Build JSON APIs with ease. Read more: https://github.com/rails/jbuilder
...
gem 'bootsnap', '>= 1.5.1', require: false   # Reduces boot times through caching; required in config/boot.rb
gem 'active_model_serializers', '~> 0.10.10' # JSON generators.
gem 'addressable', '~> 2.7.0'                # URI encoder.
gem 'dotenv-rails', '~> 2.7.6'               # Loads environment variables from `.env`.
gem 'faraday', '~> 1.1.0'                    # HTTP/REST API client library.
gem 'sidekiq', '~> 6.1.2'                    # Background processing for Ruby.
...
group :test do
  gem 'capybara', '~> 3.33.0'             # Adds support for Capybara system testing and selenium driver. Needed for RSpec feature tests.
#  gem 'chromedriver-helper', '~> 2.1.1'   # Easy installation and use of chromedriver to run system tests with Chrome
  gem 'factory_bot_rails', '~> 6.1.0'     # Factory to Rails integration.
  ...
  gem 'rspec-sidekiq', '~> 3.1.0'         # Sidekiq rspec testing.
  gem 'selenium-webdriver', '~> 3.142.0'  # Used by Capybara.
