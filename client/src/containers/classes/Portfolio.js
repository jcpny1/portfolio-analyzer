...
export function initPositionValues(portfolios) {
  portfolios.forEach(function(portfolio) {
...
export function processPrices(portfolios, trades) {
  ...
  updateSummaries(portfolios);
}

// Sort Portfolios according to supplied arguments.
export function sort(portfolios, portfolioProperty, portfolioReverseSort, positionProperty, positionReverseSort) {
...
function updateSummaries(portfolios) {
  portfolios.forEach(portfolio => {
    portfolio.totalCost   = 0.0;
    portfolio.marketValue = 0.0;
    portfolio.dayChange   = 0.0;
